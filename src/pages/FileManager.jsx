...
const FolderTreeItem = ({ folder, currentPath, onNavigate, pinnedFolders, togglePin }) => {
  ...
  const fetchSub = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${import.meta.env.DEV ? 'http://localhost:5000' : 'https://admin-v2-backend.onrender.com'}/api/cloudinary/list`, {
        method: 'POST',
...
export default function FileManager() {
  ...
  const executeDelete = async () => {
    ...
    try {
      const url = isFolder ? `${import.meta.env.DEV ? 'http://localhost:5000' : 'https://admin-v2-backend.onrender.com'}/api/cloudinary/delete-folder` : `${import.meta.env.DEV ? 'http://localhost:5000' : 'https://admin-v2-backend.onrender.com'}/api/cloudinary/delete`;
  ...
  const handleRename = async () => {
    ...
    try {
      ...
      if (isFolder) {
        url = `${import.meta.env.DEV ? 'http://localhost:5000' : 'https://admin-v2-backend.onrender.com'}/api/cloudinary/rename-folder`;
        const parts = selectedItem.path.split('/');
        parts[parts.length - 1] = newName.trim();
        body = { from_path: selectedItem.path, to_path: parts.join('/') };
      } else {
        url = `${import.meta.env.DEV ? 'http://localhost:5000' : 'https://admin-v2-backend.onrender.com'}/api/cloudinary/rename`;
        const parts = selectedItem.public_id.split('/');
        parts[parts.length - 1] = newName.trim();
  ...
  const handleDownload = async () => {
    ...
    if (isFolderMenu) {
      ...
      try {
        const res = await fetch(`${import.meta.env.DEV ? 'http://localhost:5000' : 'https://admin-v2-backend.onrender.com'}/api/cloudinary/download-folder`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
