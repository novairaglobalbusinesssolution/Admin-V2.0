...
const FolderTreeItem = ({ folder, currentPath, onNavigate, pinnedFolders, togglePin }) => {
  ...
  const fetchSub = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch('http://localhost:5000/api/cloudinary/list', {
        method: 'POST',
...
export default function FileManager() {
  ...
  const executeDelete = async () => {
    ...
    try {
      const url = isFolder ? 'http://localhost:5000/api/cloudinary/delete-folder' : 'http://localhost:5000/api/cloudinary/delete';
  ...
  const handleRename = async () => {
    ...
    try {
      ...
      if (isFolder) {
        url = 'http://localhost:5000/api/cloudinary/rename-folder';
        const parts = selectedItem.path.split('/');
        parts[parts.length - 1] = newName.trim();
        body = { from_path: selectedItem.path, to_path: parts.join('/') };
      } else {
        url = 'http://localhost:5000/api/cloudinary/rename';
        const parts = selectedItem.public_id.split('/');
        parts[parts.length - 1] = newName.trim();
  ...
  const handleDownload = async () => {
    ...
    if (isFolderMenu) {
      ...
      try {
        const res = await fetch('http://localhost:5000/api/cloudinary/download-folder', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
