...
export default function LiveCheckingMails() {
  ...
  const handleSendSingle = async (app) => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/send-live-checking-email`, {
        method: 'POST',
  ...
  const handleSendBulk = async () => {
    ...
    setSendingBulk(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/send-bulk-live-checking`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
